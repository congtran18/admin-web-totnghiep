...
import { SearchBox, AdminLoading, SortByTutor, FilterByTutor } from "app/components"
...
const TutorManage = () => {
    ...
    const queryParams = { ...queryString.parse(location.search), limit, accept: true };
    ...
    const handleFilter = (id) => {
        try {
            ...
            if (id) {
                newParams = { ...queryParams, page: 1, status: id };
            } else {
                const { status, page, ...rest } = queryParams;
                newParams = rest;
            }
                ...
                <Grid item md={22} xs={12}>
                    ...
                    <Card sx={{ p: 0.5 }}>
                        ...
                        >
                            <Box sx={{ display: 'flex' }}>
                                <SortByTutor
                                    label={"Sắp xếp gia sư"} onChange={handleSort} sort={"sort"}
                                />
                                <FilterByTutor
                                    label={"Lọc gia sư"} onChange={handleFilter} type={"status"}
                                />
                                <Button variant="contained" color="success" sx={{ display: 'flex', gap: '4px', width: '26vh', textAlign: 'center', fontSize: '14px' }} to={'/tutors/register'} as={Link}>
                                    <IconButton>
                                        timer
                                    </IconButton>
                                    Chờ phê duyệt
